...
//  Having parallel pipelines
  ...
  // Keyword differences
  val keywordDifference: Flow[((Package, Package)), ((Package, Package)), NotUsed] = Flow[(Package, Package)].map(x => (x._1.get_keyword_differences, x._2.get_keyword_differences))
  //  Sink
    ...
    x =>
      println("Analysing " + x._1.Name)
        for (i <- 0 to x._1.Version.length-1) {
          if (x._1.KeywordsDifferences(i) == "\"\""){
            println("Version: "+x._1.Version(i)+", Dependencies: "+x._1.Dependencies(i)+", DevDependencies: "+x._2.DevDependencies(i))
          }
          else {
            println("Version: " + x._1.Version(i) + ", Dependencies: " + x._1.Dependencies(i) + ", DevDependencies: " + x._2.DevDependencies(i) + ", Keywords: " + x._1.KeywordsDifferences(i))
          }
        }
  }
//  Make the graph
